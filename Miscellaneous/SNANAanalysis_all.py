...
def main():
    ...
    filenames.remove('CFA3_KEPLERCAM.LIST') 
    filenames.remove('CFA3_KEPLERCAM.IGNORE')
    filenames.remove('CFA3_KEPLERCAM.README')
    filenames.remove('CFA3_KEPLERCAM.FITRES')
    '''
    try:
        filename = sys.argv[1]
    except IndexError:
    ...
    for filename in filenames:
        ...
        for flter in filters:
            ...
            if len(b1data.mjd) > 9:
                ...
                elif len(minp) > 0 and len(minp) > 1 and minp[1][0] < (minp[0][0] + 25):
